...
from sparkmagic.utils.constants import EXPECTED_ERROR_MSG, AUTH_SSL
...
def test_add_sessions_command_parses():
    ...
    name = "-s name"
    language = "-l python"
    connection_string = "-u http://url.com -T {} -a sdf -p w".format(AUTH_SSL)
    line = " ".join([command, name, language, connection_string])

    magic.spark(line)

    add_sessions_mock.assert_called_once_with("name", Endpoint("http://url.com", AUTH_SSL, "sdf", "w"),
                                              False, {"kind": "pyspark"})
...
def test_delete_sessions_command_parses():
    ...
    command = "delete -u URL -t {} -a username -p password -i 4".format(AUTH_SSL)
    mock_method = MagicMock()
    spark_controller.delete_session_by_id = mock_method
    magic.spark(command)
    mock_method.assert_called_once_with(Endpoint("URL", AUTH_SSL, "username", "password"), 4)
...
def test_cleanup_endpoint_command_parses():
    ...
    line = "cleanup -u endp -t {} -a user -p passw"
    magic.spark(line)
    mock_method.assert_called_with(Endpoint("endp", AUTH_SSL, "user", "passw"))
