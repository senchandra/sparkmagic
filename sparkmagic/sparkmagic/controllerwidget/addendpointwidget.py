...
class AddEndpointWidget(AbstractMenuWidget):
    ...
    def __init__(self, spark_controller, ipywidget_factory, ipython_display, endpoints, endpoints_dropdown_widget,
        ...
        self.url_mapping = {constants.SPARK1: constants.SPARK1_ENDPOINT,
        ...
        if not conf.authentication_type():
            self.auth_type.layout.display = 'flex'
        else:
            self.auth_type.value = conf.authentication_type()
            self.auth_type.layout.display = 'none'

        # self.address_widget.on_trait_change(self.update_endpoint)
        self.children = [self.address_widget, self.url_widget, self.auth_type, self.user_widget, self.password_widget,
    ...
    def run(self):
        ...
        if endpoint_key == 'Other':
            endpoint = Endpoint(self.url_widget.value, self.auth_type.value, self.user_widget.value, self.password_widget.value)
